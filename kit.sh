...
roots=()
while (($#)); do case "$1" in --asset-root) roots+=("$2"); shift 2;; -h|--help) usage; exit 0;; *) break;; esac; done
...
source_component_env(){ local component=$1 file; for file in "/mnt/data/$component-kit/env.sh" "$PWD/.tools/$component/env.sh" "$ROOT/.tools/$component/env.sh"; do [[ -f "$file" ]] && { sandbox_kit_source_env_if_present "$file"; return; }; done; }

install_component(){
  local component=$1; shift || true
  local a b c; local -a group=() group2=()
  case "$component" in
    java)
      ...
      "$ROOT/java/setup.sh" "$a" ${b:+"$b"} ${c:+"$c"} "$@"; source_component_env java;;
    dotnet)
      a=$(sandbox_kit_find_one 'dotnet-sdk-linux-x64.tar.gz'); "$ROOT/dotnet/setup.sh" "$a" "$@"; source_component_env dotnet;;
    python)
      sandbox_kit_collect_archive group 'python313-linux-x86_64-wheelhouse.tar.gz' 'python313-linux-x86_64-wheelhouse.tar.gz.part*'; "$ROOT/python/setup.sh" "${group[@]}" "$@"; source_component_env python;;
    linux-tools)
      sandbox_kit_collect_archive group 'debian13-amd64-dev-debug-qemu-debs.tar.gz' 'debian13-amd64-dev-debug-qemu-debs.tar.gz.part*'; "$ROOT/linux-tools/setup.sh" "${group[@]}" "$@"; source_component_env linux-tools;;
    playwright)
      sandbox_kit_collect_archive group 'playwright-linux-browsers-bundle.tar.gz' 'playwright-linux-browsers-bundle.tar.gz.part*'; "$ROOT/playwright/setup.sh" "${group[@]}" "$@"; source_component_env playwright;;
    android-analysis)
    ...
    android-tools)
      a=$(sandbox_kit_find_one 'commandlinetools-linux-15859902_latest.zip'); b=$(sandbox_kit_find_one 'platform-tools-latest-linux.zip' optional || true); "$ROOT/android-tools/setup.sh" "$a" ${b:+"$b"} "$@";;
    unicorn)
    ...
    chrome)
      a=$(sandbox_kit_find_one 'chrome-linux64.zip'); b=$(sandbox_kit_find_one 'chromedriver-linux64.zip' optional || true); "$ROOT/chrome/setup.sh" "$a" ${b:+"$b"} "$@";;
    ghidra)
      sandbox_kit_collect_archive group 'ghidra_12.1.3_PUBLIC_20260817.zip' 'ghidra_12.1.3_PUBLIC_20260817.zip.part*'; "$ROOT/ghidra/setup.sh" "${group[@]}" "$@";;
    unity)
      a=$(sandbox_kit_find_one 'unity-cli-linux-amd64.deb'); sandbox_kit_collect_archive group 'Unity-2021.3.10f1-linux.tar.xz' 'Unity-2021.3.10f1-linux.tar.xz.part*' optional || true; "$ROOT/unity/setup.sh" "$a" "${group[@]}" "$@";;
    android-emulator)
      a=$(sandbox_kit_find_one 'platform-tools-latest-linux.zip'); sandbox_kit_collect_archive group 'emulator-linux_x64-16079175.zip' 'emulator-linux_x64-16079175.zip.part*'; "$ROOT/android-emulator/setup.sh" "$a" "${group[@]}" "$@"; sandbox_kit_collect_archive group2 'x86_64-30_r16.zip' 'x86_64-30_r16.zip.part*'; "$ROOT/android-emulator/install-system-image.sh" "${group2[@]}";;
